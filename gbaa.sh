...
gbfile=$1
cat $gbfile | sed -e '/\/translation=\"/,/\"/!d' sequence.gb | sed -e '/  gene  /,/\/gene=\".*\"/d' | sed -e 's/ //g' | sed -e 's/\//\n/g' | sed -e 's/translation="/>\n/g' | sed -e 's/\"//g' | awk '!/^>/ { printf "%s", $0; n = "\n" } /^>/ { print n $0; n = "" } END { printf "%s", n }' | sed -e '1d' > protseqs.fa
grep -n ">" protseqs.fa | awk -F ':' '{print $1}' > protnamelines
sed -e '/  CDS  /,/\/gene=\".*\"/!d' sequence.gb | grep "/gene=" | sed -e 's/.*\/gene=//g' | sed -e 's/\"//g' > protnames
k=1
